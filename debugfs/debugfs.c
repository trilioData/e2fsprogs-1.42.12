...
void do_show_super_stats(int argc, char *argv[])
{
	...
	blk64_t	blk;
	FILE 	*out;
	int	c, header_only = 0, copybitmap = 0;
        int     copydata = 0;
	...
	if (header_only) {
        ...
        if (copybitmap) {
		sprintf(cmd,  "dd if=/dev/sdb1 of=/tmp/sdb1 count=8200");
		if (0 && system(cmd))
			return;
        }
	...
	for (i = 0; i < current_fs->group_desc_count; i++) {
		...
		if (!first)
			fputs("]\n", out);
        	if (copybitmap) {
			bbitblock = ext2fs_block_bitmap_loc(current_fs, i);
			ibitblock = ext2fs_inode_bitmap_loc(current_fs, i);
			sprintf(cmd,  "dd if=/dev/sdb1 of=/tmp/sdb1 bs=4096 count=1 skip=%llu seek=%llu status=noxfer", bbitblock, bbitblock);
			if (0 && system(cmd))
				return;
			sprintf(cmd,  "dd if=/dev/sdb1 of=/tmp/sdb1 bs=4096 count=1 skip=%llu seek=%llu status=noxfer", ibitblock, ibitblock);
			if (0 && system(cmd))
				return;
		}
	}
        ...
        if (copydata) {
		blk64_t startblk = -1, length = 0;
		blk64_t totalblk = 0;

		fprintf(out, " Allocated Extents:\n");
		for (blk=current_fs->super->s_first_data_block;
			...
			if (ext2fs_test_block_bitmap2(current_fs->block_map,blk)) {
  				if (startblk == -1) {
					...
					/* continue to build the extent */
					length++;
				}
			} else {
				if (startblk != -1) {
					/* end of extent, write to output */
					char cmd[128];

					totalblk += length;

			                fprintf(out, "    startblk: %llu, length: %llu\n", startblk, length);
					if (length < 16) {
						sprintf(cmd,  "dd conv=notrunc if=/dev/sdb1 of=/tmp/sdb1 bs=4096 count=%llu skip=%llu seek=%llu", length, startblk, startblk);
						if (0 && system(cmd))
							return;
		                        	//fprintf(stderr, cmd);
					} else {
						/* Break the request to 64k boundary */
						if (startblk % 16) {
							blk64_t count = 16 - startblk % 16;

							sprintf(cmd,  "dd conv=notrunc if=/dev/sdb1 of=/tmp/sdb1 bs=4096 count=%llu skip=%llu seek=%llu", count, startblk, startblk);
							if (0 && system(cmd))
								return;
		                        		//fprintf(stderr, cmd);
							length -= count;
						...
						/* Do max number of 64 k requests*/
						if (length % 16) {
							sprintf(cmd,  "dd conv=notrunc if=/dev/sdb1 of=/tmp/sdb1 bs=65536 count=%llu skip=%llu seek=%llu", length/16, startblk/16, startblk/16);
		                        		//fprintf(stderr, cmd);
							if (0 && system(cmd))
								return;
							startblk += length/16*16;
							length = length % 16;
						}
						/* Perform any left over blocks */
						if (length) {
							sprintf(cmd,  "dd conv=notrunc if=/dev/sdb1 of=/tmp/sdb1 bs=4096 count=%llu skip=%llu seek=%llu", length, startblk, startblk);
							if (0 && system(cmd))
								return;
